...
module.exports = {
    ...
    isPermittedTo: function isPermittedTo(instance, a, b) {
        var _ = this,
            level = !(typeof a === 'object' && (a.options || a.$modelOptions)) ? a : b,
            model =   typeof a === 'object' && (a.options || a.$modelOptions) ? a : b,
    ...
    findPermitted: function findPermitted(instance, a, b) {
        var _ = this,
            ...
            where = typeof b === 'object' && b.options ? a : b,
            capitalModel = capitalize(Model.name),
            permitter = _.permissions.findPermitter(instance, Model);

        if (typeof where !== 'undefined') {
            if (typeof where === 'object') {
                where.permissionLevel = { gte: _.integerForLevel(where.permissionLevel) };
            } else {
                where = { permissionLevel: { gte: _.integerForLevel(where) }};
            }
        }

        where = { include: [{ model: permitter.Via, where: where }] };
        where.include[0].where[
            permitter.User.options === instance.$modelOptions ? permitter.userIdString : permitter.modelIdString
        ] = instance.id;

        // TODO: GROUPEDAS

        return new _.Sequelize.Promise(function (resolve, reject) {
            Model.findAll(where).then(resolve).catch(reject);
        });
    },
