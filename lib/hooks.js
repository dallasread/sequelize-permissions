...
module.exports = {
            ...
            return new _.Sequelize.Promise(function (resolve, reject) {
                if (instance[_.inheritPermsField]) {
                    _.findInheriter(instance).then(function(inheriter) {
                        ...
                        _.createImmediatePermsFor(instance, inheriter).finally(resolve);
                    }).catch(resolve);
                } else {
                    resolve();
                }
    ...
    afterDestroyUser: function afterDestroyUser(user) {
        ...
        return new _.Sequelize.Promise(function (resolve, reject) {
            _.Via.destroy({ where: { userId: user.id } }).finally(resolve);
        });
    },

    createImmediatePermsFor: function createImmediatePermsFor(instance, inheriter) {
        var _ = this;
        ...
        return new _.Sequelize.Promise(function (resolve, reject) {
            ...
            permitter.Via.findAll({
                ...
                async.eachParallel(perms, function(perm, done) {
                    var insert = {};

                    insert.permissionLevel = perm.permissionLevel;
                    insert.userId = perm.userId;
                    insert[_.inheritPermsField] = true;
                    insert[_.modelIdString] = instance.id;

                    _.Via.upsert(insert).finally(done);
                }, resolve);
            }).catch(resolve);
        });
    },

    findInheriter: function findInheriter(instance, inheriter, Models) {
        var _ = this;
        ...
        return new _.Sequelize.Promise(function (resolve, reject) {
            ...
            var Model = Models.pop(),
                getStatement = 'get' + (Model.options === instance.$modelOptions ? 'Ancestors' : capitalize(Model.options.name.singular));
            ...
            instance[getStatement]().then(function(ancestor) {
                ...
                _.findInheriter(instance, inheriter, Models).then(resolve).catch(function() {
                    resolve(inheriter);
                });
            }).catch(function() {
                resolve(inheriter);
