module.exports = {
    ...
    setUserAssociations: function setUserAssociations(Models) {
        ...
        for (var i = Models.length - 1; i >= 0; i--) {
            if (Models[i]['has' + Models[i].name + 'PermissionAssociations']) continue;

            Models[i]['has' + Models[i].name + 'PermissionAssociations'] = true;
    ...
    setViaFields: function setViaFields(Models) {
        ...
        for (var i = Models.length - 1; i >= 0; i--) {
            if (Models[i].hasViaPermissionFields) continue;

            Models[i].hasViaPermissionFields = true;
    ...
    setViaAssociations: function setViaAssociations(Models) {
        ...
        for (var i = Models.length - 1; i >= 0; i--) {
            if (Models[i].hasViaPermissionAssociations) continue;

            Models[i].hasViaPermissionAssociations = true;
